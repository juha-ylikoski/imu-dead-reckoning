...
for i in range(1,24):  
    ...
    g=[]
    a=[]
    t=np.array((list(map(float,acc[list('t')].values))))
    zer=t*0
    # Make imu
    for c in 'abc':
        g.append(np.interp( np.array((list(map(float,acc[list('t')].values)))), np.array((list(map(float,gyro[list('t')].values)))), np.array((list(map(float,gyro[list(c)].values))))))
        a.append(np.array((list(map(float,acc[list(c)].values)))))
